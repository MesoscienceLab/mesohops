...
def test_initialize():
    ...
    assert set(list(noise_param.keys()) + list(noise_corr_working.keys()) + [
        'T_AXIS', 'RAND_MODEL', 'STORE_RAW_NOISE', 'NOISE_WINDOW']) == set(
...
def test_get_noise(capsys):
    ...
    # windowed noise is as we expect: intial window.
    assert np.allclose(test_noise.get_noise(t_axis[:2]),
                       test_noise_windowed.get_noise(t_axis[:2]))
    assert np.allclose(test_noise_windowed.Z_temp,test_noise_windowed._noise[:,
                                                  :nsteps_window+1])
    ...
    assert np.allclose(test_noise_windowed.Z_temp,
    ...
    assert np.allclose(test_noise.get_noise(t_axis[-2:]),
                       test_noise_windowed.get_noise(t_axis[-2:]))
    assert np.allclose(test_noise_windowed.Z_temp,test_noise_windowed._noise[:,-2:])
    # Check that unwindowed noise does not create a noise window.
    assert np.allclose(test_noise.Z_temp, test_noise._noise)
