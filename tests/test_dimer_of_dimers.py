...
loperator_altered = np.zeros([4, 4, 4], dtype=np.float64)
gw_sysbath_altered = []
lop_list_altered = []
for i in range(nsite):
    loperator_altered[i, i, i] = np.sqrt(2)
    gw_sysbath_altered.append([g_0 / 2, w_0])
    lop_list_altered.append(sp.sparse.coo_matrix(loperator_altered[i]))
    gw_sysbath_altered.append([-1j * np.imag(g_0 / 2), 500.0])
    lop_list_altered.append(loperator_altered[i])

sys_param_altered = {
    "HAMILTONIAN": np.array(hs, dtype=np.complex128),
    "GW_SYSBATH": gw_sysbath_altered,
    "L_HIER": lop_list_altered,
    "L_NOISE1": lop_list_altered,
    "ALPHA_NOISE1": bcf_exp,
    "PARAM_NOISE1": gw_sysbath_altered,
}
...
def test_alpha():
    ...
    alpha_lap = hops.noise1._corr_func_by_lop_taxis(hops.noise1.param["T_AXIS"],list(np.arange(4)))
...
def test_hops_dynamics():
    ...
    # To re-save hard-coded dynamics if noise generation changes
    '''
    np.save(path,hops.storage.data['psi_traj'])
    '''

    traj_dod = np.load(path)
    
    assert np.allclose(hops.storage.data['psi_traj'][25], traj_dod[25], rtol=1E-5)
...
def test_hops_adaptive_dynamics_full():
    ...
    """
    for system_param in [sys_param, sys_param_altered]:
        hops_ah = HOPS(
            system_param,
            noise_param=noise_param,
            hierarchy_param={"MAXHIER": 2},
