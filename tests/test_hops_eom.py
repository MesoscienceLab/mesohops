...
# Create a linear chain model with Holstein and Peierls couplingsw
linear_chain_test_ham = np.eye(101, k=1)*50 + np.eye(101, k=-1)*50

def holstein(i, nsite):
    L2 = np.zeros([nsite,nsite], dtype=np.complex128)
    L2[i,i] = 1
    return L2
...
linear_chain_list_lop_hol = [holstein(i, 101) for i in range(101)]
linear_chain_list_lop_pei = [peierls(i, 101) for i in range(100)]
list_l_by_mode_hol = np.arange(101)
list_l_by_mode_pei = np.arange(100)
...
list_modes_hol = (np.array([20850.9 - 2000j, 40 + 0j]*len(linear_chain_list_lop_hol))
                  + np.array(list(list_l_by_mode_hol)*2)).reshape(len(
    linear_chain_list_lop_hol), 2)
linear_chain_sys_param_hol = {
    "HAMILTONIAN": linear_chain_test_ham,
    "GW_SYSBATH": list_modes_hol,
    "L_HIER": linear_chain_list_lop_hol,
    "L_NOISE1": linear_chain_list_lop_hol,
    "ALPHA_NOISE1": bcf_exp,
    "PARAM_NOISE1": list_modes_hol,
    "index_l_by_mode_abs": list_l_by_mode_hol
}

list_modes_pei = (np.array([20850.9 - 2000j, 40 + 0j]*len(linear_chain_list_lop_pei))
                  + np.array(list(list_l_by_mode_pei)*2)).reshape(len(
    linear_chain_list_lop_pei), 2)
linear_chain_sys_param_pei = {
    "HAMILTONIAN": linear_chain_test_ham,
    "GW_SYSBATH": list_modes_pei,
    "L_HIER": linear_chain_list_lop_pei,
    "L_NOISE1": linear_chain_list_lop_pei,
    "ALPHA_NOISE1": bcf_exp,
    "PARAM_NOISE1": list_modes_pei,
    "index_l_by_mode_abs": list_l_by_mode_pei
}
...
def test_eom_adaptive():
    ...
    # Linear chain case
    for linear_chain_sys_param in [linear_chain_sys_param_hol,
                                   linear_chain_sys_param_pei]:
        ...
        noise_t = np.array([noise_t_base[i][0] for i in range(len(
            linear_chain_sys_param["L_HIER"]))])
        noise2_t = np.arange(len(noise_t))
        # Note: the noise we construct here is pre-conjugation, so noise 2 needs a
        # sign of +2j, rather than -2j.
        noise_t_combined = noise_t + 2.0j * np.real(noise2_t)
