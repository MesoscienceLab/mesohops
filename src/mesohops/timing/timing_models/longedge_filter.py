...
from mesohops.timing.helper_functions.hamiltonian_generation import (
    generate_1_particle_hamiltonian)
from mesohops.timing.helper_functions.loperator_generation import (
     generate_holstein_1_particle_loperators)
...
dt = 0.1                        # Simulation time step [fs]
tmax = 2000.0                   # Time length of simulation [fs]
tau = 0.05                      # Noise time step [fs]
...
k_mats = 1                      # Number of Matsubara frequencies

# Hamiltonian Generation
H2_sys_hamiltonian = generate_1_particle_hamiltonian(nsite, V)

# L-Operator Generation
list_loperators = generate_holstein_1_particle_loperators(nsite)

# Gets the list of bath correlation function modes for each independent environment
list_dl_modes = bcf_convert_dl_to_exp_with_Matsubara(e_lambda, gamma, temp, k_mats)
...
def prepare_gw_sysbath(list_lop, list_modes):
    ...
    # Get the first and second modes
    (g_0, w_0) = list_modes[:2]
    (g_1, w_1) = list_modes[2:]

    # Append both modes to the noise lists
    for L2_ind_bath in list_lop:
        list_gw_noise.append([g_0, w_0])
        list_l_noise.append(L2_ind_bath)
        list_gw_noise.append([g_1, w_1])
        list_l_noise.append(L2_ind_bath)
...
def build_HOPS_dictionaries(list_lop, list_modes, seed=None):
    ...
    # Longedge filtration list
    list_le_filter = [False, True] * len(list_lop)
...
if delta_a > 0 or delta_s > 0:
    trajectory.make_adaptive(delta_a, delta_s, update_step=u_step)

# Define the initial state of the system
psi_0 = np.zeros(nsite)
psi_0[0] = 1.0
...
# Save Timing Data
save_dir = os.getcwd()
data_dir = f'/longedge_filter_timing/nsite-{nsite}/'
