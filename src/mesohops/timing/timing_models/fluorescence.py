...
t_2 = 200                       # t2 length of simulation [fs]
t_3 = 200                       # t3 length of simulation [fs]
...
time_dict = {"t_2": t_2, "t_3": t_3}
field_dict = {"E_1": np.array([0, 0, 1])}
site_dict = {"list_ket_sites": [1], "list_bra_sites": [1],}
...
list_modes = bcf_convert_sdl_to_exp(50, 50, 0, 300)
...
# Save Timing Data
save_dir = os.getcwd()
data_dir = f'/fluorescence_timing/nsite-{nsite}/'
if seed == 1010101010101010:
    ...
    np.save(save_dir + data_dir + f'/propagation_time_t2_seed{seed}.npy',
            dyadic_spectra.storage.metadata["LIST_PROPAGATION_TIME"][0])
    np.save(save_dir + data_dir + f'/propagation_time_t3_seed{seed}.npy',
            dyadic_spectra.storage.metadata["LIST_PROPAGATION_TIME"][1])
